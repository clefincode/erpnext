...
from frappe.utils import get_link_to_form, getdate, formatdate, flt
...
class StudentAttendance(Document):
	...
	def validate_duplication(self):
		...
		if self.course_schedule:
			...
			attendance_record = flt(frappe.db.count("Student Attendance",{
				'student': self.student,
				'student_group': self.student_group,
				'date': self.date,
				'docstatus': ['!=', 2],
		...
		if attendance_record > 0:
			...
			frappe.throw(_('already exists against the Student {0}')
				.format(frappe.bold(self.student)), title=_('Duplicate Entry'))
