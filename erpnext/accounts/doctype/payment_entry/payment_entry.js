...
frappe.ui.form.on('Payment Entry', {
	...
	setup: function(frm) {
		...
		frm.set_query("reference_doctype", "references", function() {
			if (frm.doc.party_type == "Customer") {
				...
				var doctypes = ["Expense Claim", "Journal Entry", "Instructor Invoice"];
		...
		frm.set_query("reference_name", "references", function(doc, cdt, cdn) {
			...
			const party_type_doctypes = ['Sales Invoice', 'Sales Order', 'Purchase Invoice',
				'Purchase Order', 'Expense Claim', 'Fees', 'Dunning', 'Donation', "Instructor Invoice"];
	...
	validate_reference_document: function(frm, row) {
		var _validate = function(i, row) {
			...
			if(frm.doc.party_type=="Employee" &&
				!in_list(["Expense Claim", "Journal Entry", "Instructor Invoice"], row.reference_doctype)
			) {
				frappe.model.set_value(row.doctype, row.name, "against_voucher_type", null);
				frappe.msgprint(__("Row #{0}: Reference Document Type must be one of Expense Claim, Instructor Invoice, Journal Entry", [row.idx]));
				return false;
			}
