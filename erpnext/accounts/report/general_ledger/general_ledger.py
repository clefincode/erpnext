...
def validate_filters(filters, account_details):
	...
	if filters.get("account") and filters.get("group_by") == _('Group by Account'):
		for acc in filters.get("account"):
			if account_details.get(acc).is_group == 0:
	...
	for gle in gl_entries:
		if (gle.posting_date < from_date or
			...
			if filters.get("group_by") != _('Group by Voucher (Consolidated)'):
				...
				keylist = [gle.get("voucher_type"), gle.get("voucher_no"), gle.get("account"), gle.get("gl_entry")]
...
def get_result_as_list(data, filters):
	...
	for d in data:
		...
		if d.get("party_type"):
			_party_name = "title" if d['party_type'] in ("Student", "Shareholder") else d["party_type"].lower() + "_name"
			d["party_name"] = frappe.db.get_value(d["party_type"], d["party"], _party_name)
